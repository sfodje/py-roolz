...
class TestExecuteActions:
    ...
    def test_execute_action_with_params(self):
        ...
        actor = MockActor()
        actions = [{"action": "action_with_params", "params": {"key": "value", "num": 42}}]
        
        execute_actions(actor, actions)
        
        assert actor.call_count == 1
        assert actor.executed_actions[0] == ("action_with_params", {"key": "value", "num": 42})
    
    def test_execute_action_with_both_args_and_params(self):
        """Test executing action with both positional and keyword arguments."""
        actor = MockActor()
        actions = [{
            "action": "action_with_both", 
            "args": [1, 2], 
            "params": {"key": "value"}
        }]
        ...
        assert actor.call_count == 1
        assert actor.executed_actions[0] == ("action_with_both", (1, 2), {"key": "value"})
    ...
    def test_execute_actions_missing_actor(self):
        ...
        with pytest.raises(AttributeError, match="Action method 'valid_action' not found"):
    ...
    def test_execute_actions_missing_action_method(self):
        ...
        with pytest.raises(AttributeError, match="Action method 'missing_action' not found"):
            execute_actions(actor, actions)
...
class TestValidateActions:
    ...
    def test_validate_actions_multiple_invalid_keys(self):
        ...
        actions = [{"action": "valid_action", "invalid1": "value1", "invalid2": "value2"}]
        actor = MockActor()
    ...
    def test_validate_actions_missing_action_method(self):
        ...
        assert len(errors) == 1
        assert errors[0].path == "actions[0]"
        assert errors[0].message == "Action method 'missing_action' is not defined in 'MockActor'"
